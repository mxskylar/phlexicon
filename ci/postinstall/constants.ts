export const INSTALLED_RESOURCES_DIR = "installed-resources";
export const SIGN_WRITING_ONE_D_FONT_FILE = "SuttonSignWritingOneD.ttf";
export const SIGN_WRITING_FILL_FONT_FILE = "SuttonSignWritingFill.ttf";
export const SIGN_WRITING_LINE_FONT_FILE = "SuttonSignWritingLine.ttf";

export const INSTALLED_DATA_DIR = "installed-data";
export const ISO_FILE = "iso-639-3.tab";
export const UNZIPPED_PBASE_FILES_DIR = "pbase";
export const SIGN_WRITING_DICTIONARIES_FILE_PATH = `${INSTALLED_DATA_DIR}/sign-writing-dictionaries.json`;
export const SIGN_WRITING_ALPHABETS_FILE_PATH = `${INSTALLED_DATA_DIR}/sign-writing-alphabets.json`;

export const CUSTOM_DATA_DIR = "custom-data";
// Exported from: https://docs.google.com/spreadsheets/d/1sSm0at0adMhk3PKhdvKmT_plLL1EmyYTm9-ZXtYN650/edit?usp=sharing
export const ISWA_CATEGORIES_FILE_PATH =`${CUSTOM_DATA_DIR}/iswa-categories.tsv`;
export const ISWA_SYMBOL_GROUPS_FILE_PATH =`${CUSTOM_DATA_DIR}/iswa-symbol-groups.tsv`;
export const ISWA_BASE_SYMBOLS_FILE_PATH =`${CUSTOM_DATA_DIR}/iswa-base-symbols.tsv`;